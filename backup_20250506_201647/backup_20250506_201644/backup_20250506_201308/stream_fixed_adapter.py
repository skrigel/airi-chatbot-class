...
def stream():
    ...
    def generate():
        # Initial stream setup - important for frontend compatibility
        yield json.dumps("Processing your request...") + '\n'
        time.sleep(0.3)
        ...
        if vector_store and message:
            try:
                # Send status update
                yield json.dumps("Searching repository...") + '\n'
                time.sleep(0.3)
                ...
                if docs:
                    ...
                    # Stream the response
                    yield json.dumps(response_text) + '\n'
                    time.sleep(0.2)
                    ...
                    yield json.dumps("I couldn't find any relevant documents in the repository for your query. Could you try rephrasing your question?") + '\n'
            except Exception as e:
                logger.error(f"Error retrieving documents: {str(e)}")
                yield json.dumps(f"I'm sorry, I encountered an error while searching for information: {str(e)}") + '\n'
        else:
            # Fallback if vector store is not available
            yield json.dumps("I'm sorry, but I can't access the AI Risk Repository at the moment. Please try again later.") + '\n'
